...
public class JobTypeManager {
	...
	public void loadPlugins() throws JobTypeManagerException {
		...
		// Swap the plugin set. If exception is thrown, then plugin isn't
		// swapped.
		synchronized (this) {
			pluginSet = plugins;
		}
	...
	private void loadPluginJobTypes(JobTypePluginSet plugins) throws JobTypeManagerException {
		...
		if (!jobPluginsDir.exists()) {
			logger.error(
					"Job type plugin dir " + jobTypePluginDir + " doesn't exist. Will not load any external plugins.");
			return;
		...
		try {
			...
			if (executorClass == null) {
				throw new JobExecutionException(String.format(
						"Job type '" + jobType + "' is unrecognized. Could not construct job[%s] of type[%s].",
						jobProps, jobType));
			}

			// TODO: should the logic below mirror the logic for
			// PluginLoadProps?
			...
			if (pluginLoadProps != null) {
				...
				// pluginSet.getCommonPluginLoadProps() will return null if
			...
			if (null == pluginLoadProps) {
				logger.error("pluginLoadProps is null");
